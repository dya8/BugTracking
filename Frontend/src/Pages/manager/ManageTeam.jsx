...
export default function ManageUsers({managerId}) {
  const [fetchedUsers, setFetchedUsers] = useState([]);
  const [loading, setLoading] = useState(false);
  const manId = Number(managerId); // Replace with dynamic manager ID if needed
  ...
  useEffect(() => {
    const fetchProjectUsers = async () => {
      setLoading(true);
      try {
        console.log("Fetching users for manager_id:", manId);

        // Single API call
        const response = await fetch(`http://localhost:3000/api/project-users/${manId}`);
